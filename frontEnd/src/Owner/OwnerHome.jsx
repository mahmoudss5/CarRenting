...
const COLOR_MAP = {
  blue: {
    bg: "bg-gradient-to-br from-blue-500 to-indigo-600 border-blue-400",
    icon: "text-white",
    text: "text-white",
    label: "text-white/80",
  },
  emerald: {
    bg: "bg-gradient-to-br from-emerald-400 to-teal-500 border-emerald-400",
    icon: "text-white",
    text: "text-white",
    label: "text-white/80",
  },
  amber: {
    bg: "bg-gradient-to-br from-amber-400 to-orange-500 border-amber-400",
    icon: "text-white",
    text: "text-white",
    label: "text-white/80",
  },
  violet: {
    bg: "bg-gradient-to-br from-violet-500 to-purple-600 border-violet-400",
    icon: "text-white",
    text: "text-white",
    label: "text-white/80",
  },
  red: {
    bg: "bg-gradient-to-br from-red-500 to-rose-600 border-red-400",
    icon: "text-white",
    text: "text-white",
    label: "text-white/80",
  },
...
function StatCard({ label, value, icon: Icon, color }) {
  ...
  return (
    <motion.div
      variants={fadeUp}
      className={`group relative overflow-hidden rounded-3xl p-6 shadow-[0_4px_15px_rgba(0,0,0,0.05)] border hover:shadow-xl hover:-translate-y-1 transition-all duration-300 ${c.bg}`}>
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div
          className={`p-3 rounded-2xl bg-white/20 backdrop-blur-sm group-hover:bg-white/30 transition-colors duration-300`}>
          <Icon size={20} className={c.icon} strokeWidth={2.5} />
        </div>
      </div>
      <div className="relative z-10">
        <h3 className={`font-display text-4xl font-extrabold ${c.text}`}>
          {value}
        </h3>
        <p
          className={`font-body text-sm font-semibold uppercase tracking-wider mt-1 ${c.label}`}>
          {label}
        </p>
      </div>
    </motion.div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-6 text-center bg-white rounded-3xl border border-dashed border-gray-200 col-span-full">
      ...
      <p className="font-body text-sm text-gray-500 max-w-[340px] leading-relaxed mb-6">
        Try adjusting your filters, or create a new car listing to start your
        fleet.
      </p>
      ...
      <motion.div
        ...
        <motion.div
          ...
          <div className="flex gap-3">
            <button
              onClick={onCancel}
              className="flex-1 py-3 px-4 rounded-xl font-body text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200">
              Cancel
            </button>
            <button
              onClick={onConfirm}
              disabled={isDeleting}
              className="flex-1 py-3 px-4 rounded-xl font-body text-sm font-semibold text-white bg-red-600 hover:bg-red-700 flex items-center justify-center gap-2 disabled:opacity-70">
              {isDeleting ? (
    ...
    <motion.article
      ...
      <div className="relative h-48 sm:h-52 w-full bg-gray-100 overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <div
            className={`px-3 py-1.5 rounded-full backdrop-blur-md bg-white/90 shadow-sm flex items-center gap-1.5`}>
        ...
        <div className="absolute bottom-4 right-4 bg-gray-900 text-white px-3 py-1.5 rounded-xl font-display font-bold text-sm shadow-lg">
          ${post.rentalPrice}{" "}
          <span className="text-gray-400 font-body text-xs font-normal">
            / day
          </span>
        </div>
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <p className="font-body text-sm text-gray-500 mb-4">
          {post.brand} {post.model} {post.year !== "—" && `· ${post.year}`}
        </p>

        <div className="flex flex-wrap gap-2 mb-6">
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-gray-50 border border-gray-100 text-xs font-semibold text-gray-600">
            <Car size={10} /> {post.carType}
          </span>
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-gray-50 border border-gray-100 text-xs font-semibold text-gray-600">
            <Settings2 size={10} /> {post.transmission}
          </span>
          {post.location && post.location !== "—" && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-gray-50 border border-gray-100 text-xs font-semibold text-gray-600 truncate max-w-[120px]">
              <MapPin size={10} className="shrink-0" /> {post.location}
            </span>
          )}
        </div>
        ...
        <div className="mt-auto grid grid-cols-2 gap-2 pt-4 border-t border-gray-100">
          <Link
            ...
            className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50 border border-transparent hover:border-gray-200 transition-colors">
            <Settings2 size={14} /> Edit
          </Link>
          <button
            onClick={() => onDeleteRequest(post)}
            disabled={isDeleting}
            className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold text-red-600 hover:bg-red-50 border border-transparent hover:border-red-100 transition-colors disabled:opacity-50">
            {isDeleting ? (
...
const STAT_ICONS = [Car, CheckCircle2, Hourglass, AlertTriangle];
const STAT_COLORS = ["blue", "emerald", "amber", "red"];

export default function OwnerHome() {
  const { posts, stats, isLoading, error, deletingId, deletePost, refetch } =
    useOwnerHome();
  ...
  const filteredPosts = useMemo(() => {
    return posts.filter((post) => {
      ...
      const matchStatus =
        statusFilter === "All" || post.ownerRentalStatus === statusFilter;
    ...
    <OwnerPageLayout>
      <motion.div
        ...
        <div className="relative overflow-hidden flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 p-8 sm:p-10 rounded-3xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 shadow-[0_8px_30px_rgba(79,70,229,0.2)]">
          ...
          <div className="relative z-10">
            <h1 className="font-display text-4xl sm:text-5xl font-extrabold text-white tracking-tight drop-shadow-sm">
              Fleet Overview
            </h1>
            <p className="font-body text-white/80 mt-3 text-sm sm:text-base max-w-2xl leading-relaxed">
              Manage your premium car listings, monitor performance, and expand
              your inventory right from your dashboard.
            </p>
          </div>
          <div className="relative z-10 flex flex-wrap items-center gap-3 shrink-0">
            <Link
              to="/owner"
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 text-white font-semibold hover:bg-white/20 shadow-sm transition-all text-sm">
              <LayoutDashboard size={16} /> Inbox
            </Link>
            <Link
              to="/owner/create-post"
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-indigo-700 font-bold hover:bg-gray-50 shadow-lg hover:shadow-xl transition-all text-sm">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              onClick={refetch}
              className="p-3 rounded-xl border border-gray-200 bg-white text-gray-500 hover:text-gray-900 shadow-sm transition-colors shrink-0"
              title="Refresh Listings">
              <RefreshCw
                size={18}
                className={isLoading ? "animate-spin text-gray-900" : ""}
              />
            </button>
          </div>
        </div>
        ...
        {error && !isLoading ? (
          <div className="text-center py-20 bg-red-50 rounded-3xl border border-red-100">
            ...
            <button
              onClick={refetch}
              className="px-6 py-2 bg-red-600 text-white rounded-lg font-semibold shadow hover:bg-red-700">
              Try Again
            </button>
          </div>
        ) : isLoading ? (
