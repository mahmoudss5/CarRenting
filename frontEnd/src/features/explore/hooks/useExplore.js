...
function mapCar(c) {
  ...
  return {
    ...
    pricePerDay: isNaN(pricePerDay) ? 0 : pricePerDay,
...
export function useExplore() {
  ...
  const fetchCars = useCallback(async (filters, page) => {
    ...
    try {
      ...
      let minP = Number(filters.priceRange.min);

      let maxP = Number(filters.priceRange.max);

      if (!Number.isFinite(minP) || minP < 0) minP = 0;

      if (!Number.isFinite(maxP) || maxP <= 0) maxP = undefined;

      else if (minP > maxP) [minP, maxP] = [maxP, minP];
      ...
      console.log('[useExplore] Fetching cars with params:', params);
  ...
  const sortedCars = useMemo(() => {
    ...
    let lo = Number(appliedFilters.priceRange.min);

    let hi = Number(appliedFilters.priceRange.max);
  ...
  }, [cars, sort, appliedFilters.transmission, appliedFilters.priceRange]);
